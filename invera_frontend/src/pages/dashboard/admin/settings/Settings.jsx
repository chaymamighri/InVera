      ...
      <div className="space-y-4">
        ...
        >
          <div className="space-y-4">
            ...
            <InputField label="Rôle" value={profile.role} disabled />
            <InputField label="Téléphone" value={profile.phone} onChange={val => updateProfile('phone', val)} />
        ...
        >
          <ToggleSetting label="Email" checked={settings.emailNotifications} onChange={val => updateSetting('emailNotifications', val)} />
          <ToggleSetting label="Push" checked={settings.pushNotifications} onChange={val => updateSetting('pushNotifications', val)} />
          <ToggleSetting label="Rapports Hebdomadaires" checked={settings.weeklyReports} onChange={val => updateSetting('weeklyReports', val)} />
          <ToggleSetting label="Rapports Mensuels" checked={settings.monthlyReports} onChange={val => updateSetting('monthlyReports', val)} />
        </SettingsCard>
        ...
        >
          <SelectField label="Thème" value={settings.theme} onChange={val => updateSetting('theme', val)}
            options={[{label:'Clair', value:'light'}, {label:'Sombre', value:'dark'}, {label:'Auto', value:'auto'}]} />
          <SelectField label="Langue" value={settings.language} onChange={val => updateSetting('language', val)}
            options={[{label:'Français', value:'fr'}, {label:'English', value:'en'}, {label:'العربية', value:'ar'}]} />
          <SelectField label="Format de Date" value={settings.dateFormat} onChange={val => updateSetting('dateFormat', val)}
        ...
        >
          <ToggleSetting label="Authentification à 2 facteurs" checked={settings.twoFactorAuth} onChange={val => updateSetting('twoFactorAuth', val)} />
          <SelectField label="Délai d'expiration de session" value={settings.sessionTimeout} onChange={val => updateSetting('sessionTimeout', val)}
            options={[{label:'15 min', value:'15'}, {label:'30 min', value:'30'}, {label:'1h', value:'60'}, {label:'Jamais', value:'never'}]} />
...
const SettingsCard = ({ title, icon, expanded, onToggle, children }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    >
      <div className="flex items-center gap-3">{icon}<span className="font-semibold text-gray-900">{title}</span></div>
...
const InputField = ({ label, value, onChange, disabled }) => (
  <div>
    ...
    <input
      ...
      disabled={disabled}
      className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 ${disabled ? 'bg-gray-50 text-gray-500' : ''}`}
    />
  </div>
);
