...
const StatsAchats = () => {
  ...
  const getUserInfo = () => {
    let userName = tr('userFallback');

    try {
      const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
      const adminInfo = JSON.parse(localStorage.getItem('adminInfo') || '{}');
      userName = userInfo?.nom || adminInfo?.nom || localStorage.getItem('userName') || tr('userFallback');
    } catch (e) {
  ...
  const formattedPeriodStart =
    hasDateFilter && selectedStartDate ? formatDateForDisplay(selectedStartDate) : tr('last30Days');
  const formattedPeriodEnd =
    hasDateFilter && selectedEndDate ? formatDateForDisplay(selectedEndDate) : formatDateForDisplay(currentDate);
  ...
  const exportToExcel = () => {
    ...
    const commandesData = [
      ...
      [tr('pending'), commandesATraiter.enAttente || 0],
      [tr('inProgress'), commandesATraiter.enCours || 0],
    ];
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const rows = (items) =>
        items.map((item) => `<tr><td>${item[0]}</td><td class="text-right">${item[1]}</td></tr>`).join('');
          ...
          <head>
            ...
            <style>
              * { margin: 0; padding: 0; box-sizing: border-box; }
              body { font-family: Arial, sans-serif; background: #f5f7fa; padding: 20px; line-height: 1.5; color: #1e293b; }
              ...
              .header { padding: 24px 28px; border-bottom: 1px solid #eef2f6; display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; }
              .left-section { display: flex; align-items: center; gap: 20px; }
              .logo { width: 60px; height: 60px; object-fit: contain; }
              .company-details { border-${isArabic ? 'right' : 'left'}: 1px solid #e2e8f0; padding-${isArabic ? 'right' : 'left'}: 16px; }
              .company-details p { margin: 3px 0; font-size: 11px; color: #475569; }
              .report-info { text-align: ${isArabic ? 'left' : 'right'}; }
              .report-title { font-size: 24px; font-weight: 700; color: #0f172a; }
              .muted { font-size: 11px; color: #64748b; margin-top: 4px; }
              .period-badge { display: inline-flex; padding: 6px 14px; border-radius: 9999px; font-size: 11px; font-weight: 600; background: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; margin-top: 8px; }
              ...
              .section-title { font-size: 14px; font-weight: 700; color: #1e293b; margin-bottom: 15px; }
              table { width: 100%; border-collapse: collapse; border: 1px solid #edf2f7; }
              th { background: #f8fafc; padding: 12px 10px; text-align: ${textAlign}; font-size: 11px; color: #64748b; border-bottom: 1px solid #e2e8f0; }
              td { padding: 10px; font-size: 12px; color: #334155; border-bottom: 1px solid #edf2f7; text-align: ${textAlign}; }
            ...
            <div class="dashboard-container">
              <div class="header">
                ...
                <div class="report-info">
                  <div class="report-title">${tr('reportTitle')}</div>
                  <div class="muted">${tr('exportedBy')}: ${safeUserName}</div>
                  <div class="muted">${tr('onDate', { date: formattedDate })}</div>
                  <div class="period-badge">${formattedPeriodStart} - ${formattedPeriodEnd}</div>
                </div>
              </div>
              <div class="kpi-grid">
                <div class="kpi-card"><div class="kpi-label">${tr('purchaseOrders')}</div><div class="kpi-value">${formatNumber(stats.commandes?.total)}</div></div>
                <div class="kpi-card"><div class="kpi-label">${tr('pending')}</div><div class="kpi-value">${formatNumber(stats.commandes?.enAttente)}</div></div>
                <div class="kpi-card"><div class="kpi-label">${tr('delivered')}</div><div class="kpi-value">${formatNumber(stats.commandes?.livre)}</div></div>
                <div class="kpi-card"><div class="kpi-label">${tr('stockValue')}</div><div class="kpi-value">${formatNumber(stats.stock?.valeurTotale)} DH</div></div>
              </div>
              <div class="section">
                <div class="section-title">${tr('ordersEvolutionUpper')}</div>
                <table><thead><tr><th>${tr('period')}</th><th class="text-right">${tr('count')}</th></tr></thead><tbody>${rows(evolutionCommandes.map((item) => [item.label, item.valeur]))}</tbody></table>
              </div>
              <div class="section">
                <div class="section-title">${tr('stockMovementsUpper')}</div>
                <table><thead><tr><th>${tr('period')}</th><th class="text-right">${tr('entries')}</th><th class="text-right">${tr('exits')}</th></tr></thead><tbody>${mouvementsStock.map((item) => `<tr><td>${item.label}</td><td class="text-right">${formatNumber(item.entrees)}</td><td class="text-right">${formatNumber(item.sorties)}</td></tr>`).join('')}</tbody></table>
              </div>
              <div class="section">
                <div class="section-title">${tr('categoryBreakdownUpper')}</div>
                <table><thead><tr><th>${tr('category')}</th><th class="text-right">${tr('products')}</th></tr></thead><tbody>${rows(repartitionCategories.map((item) => [item.categorie, item.nombreProduits]))}</tbody></table>
              </div>
            </div>
          </body>
        </html>`;
      ...
      alert(`${tr('pdfExportError')} ${pdfError.message}`);
    } finally {
      setExporting(false);
    }
  };
      ...
      <div className={`rounded-xl shadow-sm border p-5 transition-shadow ${isEmpty ? 'bg-gray-50 border-gray-200' : 'bg-white border-gray-100 hover:shadow-md'}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            ...
            {isEmpty && <p className="text-xs text-gray-400 mt-2">{tr('selectPeriod')}</p>}
        ...
        <div className="relative w-full overflow-x-auto">
          <svg width={chartWidth} height={chartHeight + 80} viewBox={`0 0 ${chartWidth} ${chartHeight + 80}`}>
            ...
            {data.map((item, idx) => {
              ...
              return (
                <g key={idx}>
                  <title>{`${item.label}: ${tr('entries')}: ${formatNumber(item.entrees)}, ${tr('exits')}: ${formatNumber(item.sorties)}`}</title>
                  <rect x={x} y={chartHeight - 10 - entryHeight} width={barWidth} height={Math.max(entryHeight, 3)} fill="#3B82F6" rx="4" />
                  <rect x={x + barWidth + 10} y={chartHeight - 10 - exitHeight} width={barWidth} height={Math.max(exitHeight, 3)} fill="#EF4444" rx="4" />
          ...
          <div className="flex flex-wrap justify-center gap-6 mt-4 pt-2">
            <div className="flex items-center gap-2"><div className="w-4 h-4 bg-blue-500 rounded" /><span className="text-sm text-gray-600">{tr('entriesLegend')}</span></div>
            <div className="flex items-center gap-2"><div className="w-4 h-4 bg-red-500 rounded" /><span className="text-sm text-gray-600">{tr('exitsLegend')}</span></div>
          </div>
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {!hasDateFilter ? (
              <PeriodRequired message={tr('selectPeriodForAlerts')} tr={tr} />
            ) : (
              <>
                {alertesRupture > 0 && (
                  <AlertRow color="red" title={tr('outOfStockProducts')} message={tr('affectedProducts', { count: alertesRupture })} />
                )}
                {alertesCritique > 0 && (
                  <AlertRow color="orange" title={tr('criticalStock')} message={tr('affectedProducts', { count: alertesCritique })} />
                )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          {!hasDateFilter ? (
            <PeriodRequired message={tr('selectPeriodForOrders')} tr={tr} />
          ) : (
            <div className="space-y-4">
              <OrderProcessRow
                icon={<ClockIcon className="w-5 h-5 text-yellow-600" />}
                bg="bg-yellow-100"
                title={tr('pendingValidation')}
                subtitle={tr('toApprove')}
                value={commandesATraiter.enAttente || 0}
                color="text-yellow-600"
              />
              <OrderProcessRow
                icon={<TruckIcon className="w-5 h-5 text-blue-600" />}
                bg="bg-blue-100"
                title={tr('deliveryInProgress')}
                subtitle={tr('shippedOrders')}
                value={commandesATraiter.enCours || 0}
                color="text-blue-600"
              />
            </div>
          )}
...
const PeriodRequired = ({ message, tr }) => (
...
const AlertRow = ({ color, title, message }) => {
  const colorClasses = {
...
const OrderProcessRow = ({ icon, bg, title, subtitle, value, color }) => (
