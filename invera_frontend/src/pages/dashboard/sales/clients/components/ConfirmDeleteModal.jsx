...
const ConfirmDeleteModal = ({ isOpen, onClose, onConfirm, clientName, t, isArabic }) => {
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6 transform transition-all">
          ...
          <h3 className="text-lg font-semibold text-gray-900 text-center mb-2">
            {t('salesPages.confirmDelete')}
          </h3>

          <div className="text-sm text-gray-500 text-center mb-6">
            <p>{t('salesPages.deleteClientPrompt')}</p>
            <p className="font-medium text-gray-900 mt-1">{clientName || t('salesPages.thisClient')}</p>
            <p className="mt-2 text-xs text-red-500">{t('salesPages.irreversibleAction')}</p>
          </div>
